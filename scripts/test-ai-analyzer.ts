...
async function testAIAnalyzer() {
  ...
  try {
    ...
    console.log(
      `  - 주차: ${result.has_parking === null ? "정보 없음" : result.has_parking ? "가능" : "불가"}`
    );
    console.log(
      `  - 유모차: ${result.has_stroller_access === null ? "정보 없음" : result.has_stroller_access ? "접근 가능" : "어려움"}`
    );
    console.log(
      `  - 수유실: ${result.has_nursing_room === null ? "정보 없음" : result.has_nursing_room ? "있음" : "없음"}`
    );
    console.log(
      `  - 기저귀 교환대: ${result.has_diaper_station === null ? "정보 없음" : result.has_diaper_station ? "있음" : "없음"}`
    );

    console.log("\n시설 정보:");
    console.log(
      `  - 실내: ${result.is_indoor === null ? "정보 없음" : result.is_indoor ? "예" : "아니오"}`
    );
    console.log(
      `  - 실외: ${result.is_outdoor === null ? "정보 없음" : result.is_outdoor ? "예" : "아니오"}`
    );
