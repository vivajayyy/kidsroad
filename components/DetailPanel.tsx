...
export default function DetailPanel({ event, onClickClose }: DetailPanelProps) {
  ...
  const imageUrl =
    event.firstimage ||
    event.firstimage2 ||
    "/images/default-event-thumbnail.jpg";
  const startDate = event.eventstartdate
    ? format(new Date(event.eventstartdate), "yyyy.MM.dd (eee)", { locale: ko })
    : "날짜 미정";
  const endDate = event.eventenddate
    ? format(new Date(event.eventenddate), "yyyy.MM.dd (eee)", { locale: ko })
    : "날짜 미정";

  const parentalChecklistItems = [
    { condition: event.has_parking, icon: "local_parking", label: "주차 가능" },
    {
      condition: event.has_nursing_room,
      icon: "baby_changing_station",
      label: "수유실",
    },
    {
      condition: event.has_stroller_access,
      icon: "stroller",
      label: "유모차 접근성",
    },
  ...
  return (
    <div className="relative w-full h-full bg-white dark:bg-[#161616] overflow-y-auto custom-scrollbar">
      <div className="relative w-full h-[45vh]">
        ...
        <div className="absolute top-6 right-6 z-10">
          <button
            ...
            className="bg-white/90 dark:bg-black/80 backdrop-blur p-2 rounded-full shadow-lg"
          >
      ...
      <div className="p-10">
        ...
        <div className="my-12">
          <h3 className="text-[11px] uppercase tracking-[0.2em] font-bold text-gray-400 mb-6">
            Parental Checklist
          </h3>
          <div className="grid grid-cols-2 gap-y-8 gap-x-4">
            {parentalChecklistItems
              ...
              .map((item) => (
                <div key={item.label} className="flex items-start gap-4">
                  <div className="w-10 h-10 border border-gray-100 dark:border-gray-800 flex items-center justify-center rounded-lg">
                    <span className="material-symbols-outlined text-sage-600 font-light text-[20px]">
                      {item.icon}
                    </span>
                  </div>
                  <div>
