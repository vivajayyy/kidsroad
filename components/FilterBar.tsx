...
const REGIONS = [
  '전체', '서울', '경기', '인천', '강원', '대전', '세종', '충남', '충북', 
  '부산', '대구', '울산', '경남', '경북', '광주', '전남', '전북', '제주'
];
    ...
    <div className="mb-12" ref={ref}>
      ...
      <div className="flex flex-col sm:flex-row gap-4">
        ...
        <div className="flex-1">
          <label htmlFor="category-select" className="text-xs text-gray-500 mb-2 block">카테고리</label>
          <select
            id="category-select"
        ...
        {/* Region Filter */}
        <div className="flex-1">
          <label htmlFor="region-select" className="text-xs text-gray-500 mb-2 block">지역</label>
          <select
            id="region-select"
            value={currentRegion}
          ...
          >
            {REGIONS.map(reg => (
              <option key={reg} value={reg === '전체' ? '' : reg}>{reg}</option>
            ))}
          </select>
        </div>
      </div>
