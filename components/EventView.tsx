...
export default function EventView({ events }: { events: Event[] }) {
  ...
  const getChecklist = (event: Event) => {
    ...
    if (event.has_stroller_access) checklist.push({ icon: 'stroller', label: 'Stroller Access', sub: 'Barrier-free' });
    if (event.has_nursing_room) checklist.push({ icon: 'baby_changing_station', label: 'Nursing Room', sub: 'Available' });
    if (event.has_diaper_station) checklist.push({ icon: 'stroller', label: 'Diaper Station', sub: 'Available' });
    return checklist;
  }
          ...
          {selectedEvent && (
            <>
              ...
              <div className="p-10">
                ...
                <div className="text-gray-500 text-sm leading-relaxed font-light mb-12" dangerouslySetInnerHTML={{ __html: selectedEvent.description || '' }} />
                ...
                <div className="border-t border-gray-100 dark:border-gray-800 pt-2 mb-12 space-y-0">
                  ...
                  <DetailRow label="Schedule" value={`${formatDate(selectedEvent.eventstartdate)} - ${formatDate(selectedEvent.eventenddate)}`} />
                  <DetailRow label="Fee" value={selectedEvent.usetimefestival} />
                </div>
