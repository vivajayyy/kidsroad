// utils/mapper.ts
import { FestivalItem, DetailCommonItem, FestivalIntroItem, TourImageItem, CultureIntroItem } from '../types/tour-api';
...
export function mapTourApiToEvent(
    ...
    const mapx = commonDetail?.mapx ? parseFloat(commonDetail.mapx) : (festivalItem.mapx ? parseFloat(festivalItem.mapx) : null);
    const mapy = commonDetail?.mapy ? parseFloat(commonDetail.mapy) : (festivalItem.mapy ? parseFloat(festivalItem.mapy) : null);
    ...
    const ageRanges = inferAgeRanges(festivalItem.title, commonDetail?.overview || '');
    const isIndoor = inferIsIndoor(festivalItem.title, commonDetail?.addr1 || '');
    const isOutdoor = inferIsOutdoor(festivalItem.title, commonDetail?.addr1 || '');
    ...
    return {
        ...
        createdtime: festivalItem.createdtime ? convertTourApiDateTimeToISO(festivalItem.createdtime) : null,
        modifiedtime: festivalItem.modifiedtime ? convertTourApiDateTimeToISO(festivalItem.modifiedtime) : null,
...
function convertTourApiDateTimeToISO(dateTimeString: string): string | null {
    ...
    try {
        ...
        console.error('Failed to parse TourAPI datetime string:', dateTimeString, e);
        return null;
    }
}
...
function inferIsOutdoor(title: string, addr: string): boolean | null {
    ...
    if (/실내|전시관|박물관|미술관|공연장|극장|센터/.test(lowerText)) return false;
    return null; // Unknown
}
...
function inferHasParking(parkingText: string): boolean | null {
    ...
    if (lowerText.includes('가능') || lowerText.includes('있음') || lowerText.includes('주차')) return true;
...
function inferHasStrollerAccess(strollerText: string): boolean | null {
    ...
    if (lowerText.includes('불가능') || lowerText.includes('반입불가')) return false;
    return null; // Unknown
}
