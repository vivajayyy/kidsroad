import { TourApiResponse, FestivalItem, DetailCommonItem, FestivalIntroItem, CultureIntroItem, TourImageItem } from '../types/tour-api';
...
async function fetchTourApi<T>(
    ...
    try {
        ...
        try {
            ...
            console.error(`🚨 Failed to parse JSON for endpoint ${endpoint}. Raw response text:`, rawText);
            return [];
        }
        // --- End of debugging modification ---

        if (!data.response || !data.response.header) {
            console.error(`🚨 Unexpected API response structure for endpoint ${endpoint}. Full response object:`, data);
            return [];
        }
